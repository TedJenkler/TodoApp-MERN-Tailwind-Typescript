...
function StatusSelect({ todo }) {
  ...
  return (
    <div className="relative">
      <label className="text-white text-xs mb-2">Current Status</label>
      <div onClick={handleSelect} className="flex justify-between items-center w-full h-10 border border-mediumgrey/25 py-2 px-4 cursor-pointer"> 
        <p className="bl text-white">{status ? selectedColumns.find(column => column._id === status)?.name : null}</p>
      ...
      {openMenu && 
        <div className="absolute top-[4.375rem] w-full bg-darkbg rounded-lg shadow-lg mt-1 z-10">
          {selectedColumns.map((column: any) => (
            <div
              key={column._id}
              className="flex items-center px-4 h-[2.438rem] cursor-pointer hover:bg-gray-200"
              onClick={() => handleStatusChange(column._id)}
            >
              <p className="bl text-mediumgrey">{column.name}</p>
            </div>
          ))}
        </div>
      }
